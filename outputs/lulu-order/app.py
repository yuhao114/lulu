...
<body data-admin="0">
  <div class="app">
    <header>
      <div>
        <div class="tag">真实后端登录 · 下单通知商家 · 订单状态跟踪</div>
        <h1>噜噜菜馆</h1>
        <p class="summary">顾客扫码点餐、搜索菜品、选择规格口味并提交订单；提交后商家会实时接收新订单并处理出餐状态。</p>
      </div>
      <div class="top">
      ...
      <div class="layout">
        <main>
          <section class="panel scan">
            <div class="qr"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></div>
            <div><h2>桌号 <span class="table" id="tableId"></span></h2><p>模拟扫码进入当前桌台；网址可加 <strong>?table=B12</strong> 指定桌号。</p></div>
            <button class="ghost" id="scanBtn">模拟扫码换桌</button>
          </section>
  ...
  <script>
    ...
    const state = { isAdmin: document.body.dataset.admin === '1', user: JSON.parse(localStorage.getItem('lulu_user') || 'null'), table: new URLSearchParams(location.search).get('table') || localStorage.getItem('lulu_table') || 'A03', dishes: [], orders: [], cart: [], category: '全部', query: '', seenOrders: new Set(), notify: false };
    ...
    function renderShell(){ $('#tableId').textContent=state.table; $('#userLine').textContent=state.isAdmin?'已进入商家后台':(state.user?`已登录：${state.user.name} · ${state.user.phone} · 桌号 ${state.table}`:'未登录；浏览菜单可以，下单前需要登录。'); $('#loginName').value=state.user?.name||''; $('#loginPhone').value=state.user?.phone||''; renderTabs(); renderMenu(); renderCart(); renderHistory(); if(state.isAdmin){ renderOrders(false); renderAdminDishes(); } }
    ...
    function renderAdminDishes(){ $('#adminDishes').innerHTML=state.dishes.map(d=>`<article class="card"><div class="cardtop"><div><strong>${d.name}</strong><div class="meta">${d.category} · ${money(d.price)} · 库存 ${d.stock}</div><div class="meta">${d.desc}</div></div><span class="state ${d.sold_out?'warn':'done'}">${d.sold_out?'售罄':'在售'}</span></div><div class="adminactions"><button class="mini" data-dish-act="toggle" data-dish="${d.id}">${d.sold_out?'恢复上架':'设为售罄'}</button><button class="mini" data-dish-act="stock" data-dish="${d.id}">库存</button><button class="mini" data-dish-act="price" data-dish="${d.id}">改价</button><button class="mini" data-dish-act="delete" data-dish="${d.id}">删除</button></div></article>`).join(''); }
    function notifyMerchant(o){ if(!state.notify)return; try{ new AudioContext().createOscillator().start(); }catch(e){} if('Notification' in window && Notification.permission==='granted') new Notification('噜噜菜馆新订单', {body:`${o.table_no} ${o.customer_name} ${money(o.total)}`}); alert(`新订单：${o.table_no} ${o.customer_name} ${money(o.total)}`); }
    ...
    $('#scanBtn').onclick=()=>{ const t=prompt('请输入扫码后的桌号',state.table); if(t){state.table=t.trim();localStorage.setItem('lulu_table',state.table);renderShell();} };
    ...
    $('#addDish').onclick=async()=>{ const body={name:$('#dishName').value.trim(),category:$('#dishCategory').value.trim(),price:Number($('#dishPrice').value),desc:$('#dishDesc').value.trim()}; if(!body.name||!body.category||!body.price){alert('请填写菜名、分类和价格');return;} await api('/api/dishes',{method:'POST',body:JSON.stringify(body)}); $('#dishName').value=$('#dishCategory').value=$('#dishPrice').value=$('#dishDesc').value=''; loadMenu(); };
    ...
    $('#adminDishes').onclick=async e=>{ const b=e.target.closest('[data-dish-act]'); if(!b)return; let body={action:b.dataset.dishAct}; if(body.action==='stock')body.stock=Number(prompt('库存数量')); if(body.action==='price')body.price=Number(prompt('新价格')); await api('/api/dishes/'+b.dataset.dish,{method:body.action==='delete'?'DELETE':'PATCH',body:JSON.stringify(body)}); loadMenu(); };
